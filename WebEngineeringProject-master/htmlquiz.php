...
<?php 				
		...
		$fetchqry2 = "UPDATE `htmlquiz` SET `userans`='$userselected' WHERE `id`=$c-1"; 
...
<table><?php if(isset($c)) {   $fetchqry = "SELECT * FROM `htmlquiz` where id='$c'"; 
...
<tr><td><h3><?php echo @$row['que'];?></h3></td></tr> <?php if($_SESSION['clicks'] > 0 && $_SESSION['clicks'] < 26){ ?>
 ...
 if($_SESSION['clicks']>25){ 
	$qry3 = "SELECT `ans`, `userans` FROM `htmlquiz`;";
